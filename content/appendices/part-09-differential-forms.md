...
我们选定标准体积形式 $\mathrm{vol}=dx\wedge dy\wedge dz$，并约定向量场
$F=(A,B,C)$ 对应 $\iota_F\mathrm{vol}=\beta$。这正是通量所需的 2-形式。
...
因此“梯度的旋度为零”和“旋度的散度为零”都是 $d^2=0$ 的坐标影子。
...
在本书处理的具体 Euclid 区域、曲线和曲面上，统一口号是

\[
\int_{\partial M}\omega=\int_M d\omega.
\]

这里 $M$ 与 $\partial M$ 必须带相容取向，形式的次数也要匹配。下表只是对照，不替代
第 39–41 章对各经典公式的条件检查与证明。
...
选读练习：分别把 $f=xyz$ 的 $df$ 与
$\alpha=z\,dx+x\,dy+y\,dz$ 的 $d\alpha$ 写出，并用 $d^2f=0$ 检查混合偏导项怎样成对抵消。
