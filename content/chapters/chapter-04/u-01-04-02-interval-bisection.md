...
learning_goals:
- 用平方比较选择保留根号二的半区间
- 证明二分过程保持端点不变量并将区间长度减半
- 由区间长度推出中点近似的显式误差证书
- 区分平方根二的专门二分过程与一般连续函数求根
content_standard: 1
---
...
若 $m_n^2<2$，则 $0<m_n<\sqrt2<b_n$，故选 $(m_n,b_n)$ 后有

$$
a_{n+1}=m_n,b_{n+1}=b_n\in\mathbb Q,
\qquad 0<a_{n+1}=m_n<\sqrt2<b_n=b_{n+1}.
$$

若 $m_n^2>2$，则 $0<a_n<\sqrt2<m_n$，故选 $(a_n,m_n)$ 后有

$$
a_{n+1}=a_n,b_{n+1}=m_n\in\mathbb Q,
\qquad 0<a_{n+1}=a_n<\sqrt2<m_n=b_{n+1}.
$$
...
**完整解答。** 设 $0<a_0<\sqrt2<b_0$，且 $a_0,b_0\in\mathbb Q$。按本单元规则构造 $a_n,b_n$。完整证明：每一步规则都有且只有一种可用选择；对任意 $n$ 有 $a_n,b_n\in\mathbb Q$ 与 $0<a_n<\sqrt2<b_n$；并证明输出中点 $m_n$ 的误差至多为 $(b_0-a_0)/2^{n+1}$。

??? note "答案"

    端点初始为有理数，更新只取平均或保留端点，归纳得 $a_n,b_n,m_n$ 都为有理数。由于 $\sqrt2$ 无理，$m_n\ne\sqrt2$，从而 $m_n^2\ne2$；恰有 $m_n^2<2$ 或 $m_n^2>2$ 之一成立，规则有且只有一种选择。

    初始端点属于 $\mathbb Q$ 且满足 $0<a_0<\sqrt2<b_0$。假设 $a_n,b_n\in\mathbb Q$ 与 $0<a_n<\sqrt2<b_n$。则 $m_n=(a_n+b_n)/2\in\mathbb Q$，并且中点定义给出 $a_n<m_n<b_n$，尤其 $m_n>0$。由于 $m_n\ne\sqrt2$，两种平方比较中恰有一种成立。于是可在正数上比较平方：若 $m_n^2<2$，则 $m_n<\sqrt2$，新端点 $(m_n,b_n)$ 仍属 $\mathbb Q$ 且满足 $0<a_{n+1}=m_n<\sqrt2<b_n=b_{n+1}$。若 $m_n^2>2$，同理 $m_n>\sqrt2$，新端点 $(a_n,m_n)$ 仍属 $\mathbb Q$ 且满足 $0<a_{n+1}=a_n<\sqrt2<m_n=b_{n+1}$。归纳完成。
