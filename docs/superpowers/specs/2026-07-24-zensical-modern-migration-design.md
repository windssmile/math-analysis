...
3. 主题使用 `theme.variant: modern`。原有 Material DOM 专用模板覆写不作为兼容目标；学习单元元数据卡片改为 Markdown 生成的可移植内容，减少模板引擎耦合。
...
每页 front matter 保留 `unit_id`、学时、先备、能力和学习目标。为避免 Zensical 与 Material 模板 API 的耦合，单元卡片不再依赖 `overrides/main.html` 或 `partials/unit-meta.html`；页面保持 front matter 供检查器与后续主题扩展读取。`extra.css` 只处理行长、中文排版、数学/代码溢出和移动端留白，不覆盖主题结构或颜色系统。
...
- 旧 Material 模板覆写被移除后，构建没有未使用依赖，且单元信息不会消失为不可读的空白结构。
