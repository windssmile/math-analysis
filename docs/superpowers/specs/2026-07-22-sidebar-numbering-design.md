...
sidebar 和 `project.render` 直接维护在 `_quarto.yml` 中，不增加构建时生成步骤。新增校验逐项比对 render 列表、侧栏链接和单元注册表，确保路径、标题、章归属和章内次序一致。
...
9. `make verify` 的纲要、注册表、Quarto 全站渲染、内部链接和必需锚点检查全部通过。
